...
## Corpus Check
- 50 files · ~283,666 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 534 nodes · 948 edges · 35 communities (32 shown, 3 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 17 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f10b9c3d`
...
3. `calcular()` - 13 edges
4. `_initCalcView()` - 12 edges
5. `Design: Precificador Multi-Plataforma PRO 2026` - 12 edges
6. `Edge Function: send-invite` - 12 edges
7. `getCurrentUserId()` - 11 edges
8. `_loadAndRender()` - 11 edges
9. `_updatePlatformaAvisoFaixa()` - 10 edges
10. `initAuth()` - 10 edges

## Surprising Connections (you probably didn't know these)
- `testCenario()` --calls--> `calcular()`  [EXTRACTED]
  tests/test-frete-condicional.js → js/calculator.js
- `processarLinha()` --calls--> `calcular()`  [EXTRACTED]
  js/lote.js → js/calculator.js
- `processarLinha()` --calls--> `mapPlataformaToInputs()`  [EXTRACTED]
  js/lote.js → js/calculator.js
...
Cohesion: 0.07
Nodes (62): _ALIQUOTAS_PRESUMIDO, _ALIQUOTAS_SIMPLES, calcular(), calcularComDesconto(), calcularFretePorRegra(), comparar(), identificarFaixa(), mapPlataformaToInputs() (+54 more)
...
Cohesion: 0.17
Nodes (18): boot(), PLATAFORMAS, isLoggedIn(), isOnboardingComplete(), getViews(), HIDE_HEADER_ROUTES, initRouter(), navigate() (+10 more)
...
Cohesion: 0.16
Nodes (21): _baixarResultados(), _baixarTemplate(), BRL, _defaultTipoVendedor(), _downloadCSV(), _esc(), _handleFileUpload(), initLote() (+13 more)
...
Cohesion: 0.09
Nodes (48): _esc(), _handleSendInvite(), initAdminPanel(), _isValidEmail(), _loadConvidados(), _loadLeadScore(), _auth, AUTH_LISTENERS (+40 more)

## Knowledge Gaps
- **205 isolated node(s):** `_ALIQUOTAS_SIMPLES`, `_ALIQUOTAS_PRESUMIDO`, `_PLATAFORMAS`, `_REGIME_LABEL`, `_ATIVIDADE_LABEL` (+200 more)
...
- **Why does `calcular()` connect `Calculator State Management` to `Formatting & Export`?**
...
- **What connects `_ALIQUOTAS_SIMPLES`, `_ALIQUOTAS_PRESUMIDO`, `_PLATAFORMAS` to the rest of the system?**
